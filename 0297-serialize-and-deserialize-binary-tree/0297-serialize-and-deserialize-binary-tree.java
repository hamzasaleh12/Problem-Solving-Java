...
public class Codec {
    ...
    public TreeNode deserialize(String data) {
        ...
        while(i < tokens.length && !queue.isEmpty()){
            ...
            parent.left = (tokens[i].equals("null")) ? null : new TreeNode(Integer.parseInt(tokens[i]));
            if(parent.left != null) queue.add(parent.left);
            i++;

            if(i < tokens.length){
                parent.right = (tokens[i].equals("null")) ? null : new TreeNode(Integer.parseInt(tokens[i]));
